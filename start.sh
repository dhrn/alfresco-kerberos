...
docker exec -ti kerberos kadmin.local -q "addprinc -kvno 0 -pw password -x dn=uid=httpprocess,ou=People,dc=example,dc=com HTTP/process"
docker exec -ti kerberos kadmin.local -q "ktadd -k process.keytab HTTP/process"
